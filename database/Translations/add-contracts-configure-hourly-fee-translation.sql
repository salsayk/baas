-- Incremental: contracts grid — "Configure hourly fee" action (EN/HE).
-- Run: node database/run-sql.mjs database/Translations/add-contracts-configure-hourly-fee-translation.sql
...
seed(source_text, en_text, he_text) AS (
  VALUES
    ('Configure hourly fee', 'Configure hourly fee', 'הגדר תעריף שעתי'),
    ('Close', 'Close', 'סגור')
)
