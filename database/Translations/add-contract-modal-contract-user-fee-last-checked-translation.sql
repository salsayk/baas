-- Incremental: contract modal "last checked" status text for Contract User Fee block.
-- Run: node database/run-sql.mjs database/Translations/add-contract-modal-contract-user-fee-last-checked-translation.sql
...
seed(source_text, en_text, he_text) AS (
  VALUES
    ('Last checked just now', 'Last checked just now', 'נבדק כעת')
)
