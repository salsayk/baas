...
CREATE TABLE IF NOT EXISTS contract_milestones_data_for_success (
  ...
  CONSTRAINT contract_milestones_data_for_success_min_max_payment_chk
    CHECK (
      ...
      OR max_payment_amount IS NULL
      OR min_payment_amount <= max_payment_amount
    ),
...
CREATE TRIGGER trigger_set_contract_milestone_success_seq_if_missing
  ...
  FOR EACH ROW
  EXECUTE PROCEDURE set_contract_milestone_success_seq_if_missing();

CREATE OR REPLACE FUNCTION validate_contract_milestone_success_seq_order()
RETURNS TRIGGER AS $$
DECLARE
...
CREATE TRIGGER trigger_validate_contract_milestone_success_seq_order
  BEFORE INSERT ON contract_milestones_data_for_success
  FOR EACH ROW
  EXECUTE PROCEDURE validate_contract_milestone_success_seq_order();
